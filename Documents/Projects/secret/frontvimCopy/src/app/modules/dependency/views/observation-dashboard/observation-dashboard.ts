...
@Component({
  selector: "observation-dashboard",
  templateUrl: "./observation-dashboard.html",
  styleUrls: ["./observation-dashboard.scss"],
})
export class ObservationDashboardComponent implements OnInit {
  ...
  private getKpiCountList(): void {
    this.trackingFlowService.getKpiCountList(SolicitudPanelFunctions.validateKeyOnSelectedQ(this.selectedApoyo),(data: any) => {
        ...
        this.kpiList.set("observation", data.observation);
      }
    );
  }
  ...
  public configViewMiniKpi(supportType: any = '-', selectedStatus: string = 'En observación', activeRol: any = this.session.getActiveRole) {
  ...
  private getApplicationList(): void {
    ...
    this.applicationService.listSolicitud(
      this.paginator.page,
      'En observación', //this.selectedStatus,
  ...
  public onApplicantCardClick(data_event: any): void {
    if (data_event["estatusActual"]["descripcion"] == "En observación") {
